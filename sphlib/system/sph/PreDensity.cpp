...
namespace rtps
{
    PreDensity::PreDensity(std::string path, CL* cli_ )
    {
        cli = cli_;
     
        try
        {
            path = path + "/pre_density.cl";
            k_density = Kernel(cli, path, "pre_density_update");
            std::cout << "Load PreDensity kernel" << std::endl;

        }
        catch (cl::Error er)
        {
            printf("ERROR(PreDensity): %s(%s)\n", er.what(), oclErrorString(er.err()));
        }


    }
    void PreDensity::execute(int num,
					Buffer<float4>& pos_u,
                    Buffer<float4>& pos_s,
                    Buffer<float>& dens_s,
                    //output
    ...
    { 
        int iarg = 0;
		k_density.setArg(iarg++, pos_u.getDevicePtr());
